...
FROM defects d
JOIN inspections i ON d.inspection_id = [i.id](http://i.id)
...
FROM products p
JOIN batches b ON p.batch_id = [b.id](http://b.id)
JOIN inspections i ON i.product_id = [p.id](http://p.id)
LEFT JOIN defects d ON d.inspection_id = [i.id](http://i.id)
...
public:
    ...
    bool insert(InspectionRecord& record) {
        ...
        try {
            ...
            [record.id](http://record.id) = query.lastInsertId().toLongLong();
            ...
            if (!record.defects.empty()) {
                ...
                for (auto& defect : record.defects) {
                    defect.inspectionId = [record.id](http://record.id);
                    ...
                    [defect.id](http://defect.id) = query.lastInsertId().toLongLong();
                }
            }
    ...
    DailyStats getTodayStats() {
        ...
        if ([query.next](http://query.next)()) {
            [stats.total](http://stats.total) = query.value("total").toInt();
            ...
            stats.yieldRate = [stats.total](http://stats.total) > 0 
                ? (double)stats.okCount / [stats.total](http://stats.total) * 100 : 0;
        }
